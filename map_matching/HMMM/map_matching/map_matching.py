...
class MapMatching(viterbi_path.ViterbiSearch):
    ...
    def calculate_transition_costs(self, source, targets):
        ...
        for target, (path, route_distance) in zip(targets, route_results):
            ...
            if ALGORITHM.name =='HMM':
                # delta = abs(math.log(max(1, route_distance)) - math.log(max(1, great_circle_distance)))
                delta = abs(route_distance - great_circle_distance)
                costs.append(delta / self.beta)
            elif ALGORITHM.name =='ST':
                delta = abs(math.log(max(1, route_distance)) - math.log(max(1, great_circle_distance)))
                costs.append(delta)
            elif ALGORITHM.name =='FV':
                delta = 2 * self.sigma_z * self.sigma_z * route_distance/self.beta
                costs.append(delta)
    ...
    def calculate_emission_cost(self, candidate):
        ...
        if ALGORITHM.name =='FV':
            return distance * distance * candidate.interval

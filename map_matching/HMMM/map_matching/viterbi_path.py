...
def _wrap_candidates(raw_candidates):
    ...
    for timestamp, (_, candidates) in enumerate(groups):
        result.append([CandidateWrapper(id=next(id), timestamp=timestamp, body=candidate)
...
def heap_str(pqueue):
    ...
    if len(pqueue) == 0:
        return '[]'
    for cost, cand, prev_cand in pqueue:
        prev_id = prev_cand.id if prev_cand is not None else '-1'
        p_str.append('(%.2f,%s,%s)'%(cost, cand.id, prev_id))
    return '[' + ','.join(p_str) + ']'

def remove_byid(state, id_):
    for c in state:
        if c.id == id_:
            break
    state.remove(c)
...
class ViterbiSearch(object):
    ...
    def search_winners(self, states):
        ...
        while True:
            ...
            logger.debug('Before pop:' + heap_str(pqueue) +' scanned:' + str(list(scanned_candidates.keys())))
            ...
            logger.debug('After pop:' + heap_str(pqueue) + ' scanned:' + str(list(scanned_candidates.keys())))

            if winner is None or winner.timestamp < timestamp:
                new_start = winner is None
                winner = candidate
                logger.debug('winner.id=%s, winner.timestamp=%s, start=%s' % (winner.id, winner.timestamp, new_start))
                yield winner, scanned_candidates, new_start

            # Remove the scanned candidate from current state
            state = states[timestamp]
            state.remove(candidate)
            logger.debug('cand_id=%s' % (candidate.id))
            logger.debug('State:' + str([c.id for c in state]))
            ...
            if not state:
                ...
                heapq.heapify(pqueue)
                logger.debug('New State:' + heap_str(pqueue))
            ...
            logger.debug('Next state:' + str([c.id for c in next_state]))
            ...
            for next_candidate, transition_cost in zip(next_state, transition_costs):
                ...
                if timestamp in [0, 1, 2]:
                    logger.debug('%s,%s' % (next_candidate.id, transition_cost))
                ...
                # if next_candidate.timestamp == 3:
                #     print next_cost_sofar, transition_cost, emission_cost, next_candidate.body.edge.id

                heapq.heappush(pqueue, (next_cost_sofar, next_candidate, candidate))
            logger.debug('Transition:' + heap_str(pqueue))
            logger.debug('------------------------------------------------')
    ...
    def offline_search(self, candidates):
        ...
        # for t in groups:
        #     print 'id=%s,timestamp=%s'%(t[0].id, t[0].timestamp)
        #     q_str = []
        #     for q in t:
        #         q_str.append('id=%s,timestamp=%s,((m.timestp=%s),edge_id=%s,distance=%s)' % (q.id, q.timestamp, q.body.measurement.id, q.body.edge.id, q.body.distance))
        #     print '[' + '------'.join(q_str) + ']'
